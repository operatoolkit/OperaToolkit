...
void ofApp::setup() {
    ...
    gui.add(oscillate.set("oscillate?", false));
    gui.add(delay.set("delayAmt", 0, 0, 300));
...
void ofApp::draw() {
    ...
    if (indexDraw >= 0)
    {
        ...
        int idx;
        if (oscillate) { // oscillate99
            int offset = ofMap(sin(0.001*ofGetFrameNum()), -1, 1, 0, 300);
            idx = (indexDraw + fbo.size() - offset) % fbo.size();
        }
        else { //simple delay
            idx = (indexDraw + fbo.size() - delay) % fbo.size();
        }
